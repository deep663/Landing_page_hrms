...
const Nav = () => {
    ...
    const toggleModel = () => {
        setIsModelActive(!isModelActive);
    };
    return (
        <>
            <nav className="bg-white px-4 flex justify-between items-center border-b border-gray-200 shadow-md">
                ...
                <button onClick={toggleModel} className="text-blue-600 border border-blue-600 px-6 py-2 rounded-full hover:bg-blue-600  hover:text-white transition-colors duration-300">
                    Log In
                </button>
            </nav>
            <div className={`${isModelActive ? 'block' : 'hidden'} `}>
                <UserLoginForm onClose={toggleModel} />
            </div>
        </>
    )
};
